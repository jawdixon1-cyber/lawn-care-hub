...
export async function jobberQuery(query, variables = {}, retryCount = 0) {
  ...
  if (json.errors?.length) {
    ...
    // Surface extensions (cost/maxCost/code) so we can see *why* it throttled.
    const ext = err.extensions ? ` ${JSON.stringify(err.extensions)}` : '';
    // Auto-retry throttle errors with a delay (up to 2 retries)
    // Don't retry throttle — fail fast, let the frontend handle it
    throw new Error(`Jobber GraphQL: ${msg}${ext}`);
  }
  return json.data;
}
